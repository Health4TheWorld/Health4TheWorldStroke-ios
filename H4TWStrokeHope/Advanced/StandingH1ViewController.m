//
//  StandingH1ViewController.m
//  H4TWStrokeHope
//
//  Created by Maitri Mehta on 1/16/18.
...
@interface StandingH1ViewController ()
@property (strong, nonatomic) IBOutlet UIScrollView *scrollView;
@property (strong, nonatomic) IBOutlet UIView *contentView;
@property (strong, nonatomic) UILabel *welcomeMessage;
@end

@implementation StandingH1ViewController
...
- (void)setUpView {
    ...
    self.welcomeMessage.text = NSLocalizedString(@"Exercises.balanceTraining.Standing.H1", nil);
    ...
    HomeButton *h2 = [[HomeButton alloc] initWithText:@"2" withFrame:CGRectMake(([UIScreen mainScreen].bounds.size.width/ 2) + (SPACE_BETWEEN_CELLS / 2), startingY, cellWidth, cellHeight)];
    [h2 addImageCentered:[UIImage imageNamed:BT_Standing_3730]];
    ...
    [h3 addImageCentered:[UIImage imageNamed:BT_Standing_3731]];
    [h3 addTarget:self action:@selector(b3Pressed) forControlEvents:UIControlEventTouchUpInside];
    cellWidth = ([UIScreen mainScreen].bounds.size.width / 2) - (SPACE_BETWEEN_CELLS) - (SPACE_BETWEEN_CELLS / 2);
    cellHeight = cellWidth;
    HomeButton *h4 = [[HomeButton alloc] initWithText:@"4" withFrame:CGRectMake(([UIScreen mainScreen].bounds.size.width/ 2) + (SPACE_BETWEEN_CELLS / 2), startingY, cellWidth, cellHeight)];
    [h4 addImageCentered:[UIImage imageNamed:BT_Standing_3732]];
    [h4 addTarget:self action:@selector(b4Pressed) forControlEvents:UIControlEventTouchUpInside];

    
    [self.contentView addSubview: h1];
    [self.contentView addSubview: h2];
    [self.contentView addSubview: h3];
    [self.contentView addSubview: h4];
